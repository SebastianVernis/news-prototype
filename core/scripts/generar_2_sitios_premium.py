...
for site_num, (config, noticias_sitio) in enumerate(zip(sitios_config, [noticias_sitio_1, noticias_sitio_2]), 1):
    ...
    # Crear directorio
    site_dir = OUTPUT_SITES_DIR / f'premium_site_{site_num}'
...
📁 Sitios generados:
   • output/sites/premium_site_1/
   • output/sites/premium_site_2/

🌐 Para servir:
   Terminal 1: python3 -m http.server 9001 --directory output/sites/premium_site_1
   Terminal 2: python3 -m http.server 9002 --directory output/sites/premium_site_2
...
print("="*70)
OUTPUT_SITES_DIR = Path(__file__).resolve().parents[2] / "output" / "sites"
