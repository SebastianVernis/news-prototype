...
try:
    ...
    article_expander_module = import_module_from_file(
        "article_expander", current_dir / "article-expander.py"
    )
    ...
    generate_images_unified_module = import_module_from_file(
        "generate_images_unified", current_dir / "generate-images-unified.py"
    )
    ...
    from layout_generator import HTMLLayoutBuilder, LayoutGenerator
...
class MasterOrchestrator:
    ...
    def __init__(self, output_base_dir: str = None, usar_api_whois: bool = False):
        ...
        """
        # Usar rutas absolutas basadas en la ubicación del repo
        ...
        if output_base_dir is None:
            self.output_base_dir = output_dir / "generated_sites"
        else:
        ...
        self.data_dir = content_dir / "data"
        self.templates_dir = content_dir / "templates"
    ...
    def paso_1_descargar_noticias(
        self, num_noticias: int = 50, force_download: bool = False
    ) -> List[Dict]:
        """
    ...
    def paso_2_parafrasear_noticias(self, noticias: List[Dict]) -> List[Dict]:
        ...
        self.log("=" * 70)
        self.log(
            "PASO 2: Parafraseando Artículos Principales (Blackbox AI)", "PROGRESS"
        )
        self.log("=" * 70)
        ...
        self.log(
            f"Parafraseando {len(noticias_principales)} artículos con Blackbox AI..."
        )
        ...
        if len(self.blackbox_parallel.api_configs) >= 2:
            ...
            noticias_parafraseadas = self.blackbox_parallel.parafrasear_lote_paralelo(
                noticias_principales, max_workers=2
            )
        else:
            ...
            for noticia_idx, noticia in enumerate(noticias_principales, 1):
                try:
                    ...
                    self.log(
                        f"  [{noticia_idx}/{len(noticias_principales)}] {noticia.get('title', '')[:50]}..."
                    )

                    paraphrased = self.paraphraser.paraphrase_article(
                        noticia, style=style
                    )
                    ...
                    if not hasattr(self, "_metadata_cache"):
                        self._metadata_cache = (
                            self.name_generator.generar_nombre_completo()
                        )
                    site_name = self._metadata_cache.get("nombre", "Noticias")

                    paraphrased["author"] = paraphrased.get(
                        "author"
                    ) or self.legal_generator.generar_autor_aleatorio(site_name)
        ...
        self.log(
            f"Artículos principales completados: {len(noticias_parafraseadas)}",
            "SUCCESS",
        )
    ...
    def paso_2_parafraseo_linguistico(self, noticias: List[Dict]) -> List[Dict]:
        ...
        self.log("=" * 70)
        self.log(
            "PASO 2: Parafraseo Lingüístico (Offline - Sin IA Generativa)", "PROGRESS"
        )
        self.log("=" * 70)
        ...
        for idx, noticia in enumerate(noticias, 1):
            self.log(
                f"  [{idx}/{len(noticias)}] Reescritura lingüística: {noticia.get('title', '')[:50]}..."
            )
    ...
    def paso_2_1_generar_placeholders(
        self, noticias_restantes: List[Dict]
    ) -> List[Dict]:
        """
        ...
        self.log(
            f"Parafraseando {len(noticias_para_placeholders)} placeholders con Gemini..."
        )
        self.log("(Parafraseo rápido: título + descripción + 3-4 párrafos)")

        placeholders = self.gemini_paraphraser.parafrasear_lote_paralelo(
            noticias_para_placeholders, max_workers=3, delay_between_batches=0.5
        )
    ...
    def paso_2_5_categorizar_noticias(
        self, noticias: List[Dict], use_ai: bool = True
    ) -> List[Dict]:
        """
        ...
        noticias_categorizadas = self.categorizador.categorizar_lote(
            noticias, use_ai=use_ai, batch_delay=0.3
        )

        self.log(
            f"Categorización completada: {len(noticias_categorizadas)} artículos categorizados",
            "SUCCESS",
        )
        return noticias_categorizadas

    def paso_3_generar_imagenes(
        self, noticias: List[Dict], site_num: int
    ) -> Dict[str, str]:
        """
...
No text, no watermarks."""
                ...
                self.log(
                    f"  [{idx}/{len(noticias)}] Descargando imagen: {title[:50]}...",
                    "PROGRESS",
                )
                ...
                image_path = self.image_generator.generate_image(
                    prompt, article_id, idx, article=noticia
                )
        ...
        self.log(
            f"Generación de imágenes completada: {self.stats['imagenes_generadas']} imágenes",
            "SUCCESS",
        )
        return imagenes

    def paso_4_crear_metadata_sitios(
        self, num_sitios: int, verificar_dominios: bool = False
    ) -> List[Dict]:
        """
    ...
    def paso_5_generar_logos(self, sites_metadata: List[Dict]) -> Dict[int, str]:
        ...
        for idx, metadata in enumerate(sites_metadata, 1):
            try:
                ...
                self.log(
                    f"  [{idx}/{len(sites_metadata)}] Generando logo SVG: {site_name}",
                    "PROGRESS",
                )
                ...
                try:
                    ...
                    self.log(
                        f"    ✓ Logo SVG: {logo_result['logo_type']} ({logo_result['font_used']})",
                        "SUCCESS",
                    )
                except Exception as e:
                    self.log(f"⚠️  Error generando logo SVG: {e}", "WARNING")
                    # Fallback: crear logo tipográfico simple
                    fallback_svg = self.logo_generator.generate_typographic_logo(
                        site_name, colors["primary"]
                    )
    ...
    def paso_6_generar_templates_css(
        self, num_sitios: int, sites_metadata: List[Dict] = None
    ) -> List[Dict]:
        """
    ...
    def paso_7_generar_sitios_html(
        self,
        ...
        try:
            ...
            preloader_code = self.preloader_generator.generar_preloader_completo(
                preloader_tipo, colores_preloader, logo_url=logo_path
            )
            index_html = self.preloader_generator.inyectar_en_html(
                index_html, preloader_code
            )
            ...
            self._generar_paginas_articulos(
                site_dir, noticias, metadata, template_info, idx, logo_path
            )
            ...
            sitios_generados.append(
                {"index_path": str(index_path), "site_dir": site_dir, "site_num": idx}
            )
    ...
    def _generar_sidebar_articulos(
        self, otras_noticias: List[Dict], metadata: Dict
    ) -> str:
        """
...
{"".join(items_html)}
    ...
    def _formatear_contenido_html(self, texto: str) -> str:
        ...
        # Si no hay dobles saltos de línea, intentar dividir por saltos simples
        # pero solo si hay múltiples líneas
        if "\n\n" not in texto and "\n" in texto:
            # Si hay saltos simples pero no dobles, asumir que cada línea es un párrafo
            parrafos = [p.strip() for p in texto.split("\n") if p.strip()]
        else:
            # Dividir por líneas vacías (doble salto de línea)
            parrafos = [p.strip() for p in texto.split("\n\n") if p.strip()]

        # Si solo hay un párrafo y es muy largo, dividirlo en párrafos más pequeños
        # pero asegurándonos de no cortar oraciones a la mitad
        if len(parrafos) == 1 and len(parrafos[0].split()) > 150:
            # Dividir párrafo largo en segmentos de ~100 palabras, pero respetando oraciones completas
            palabras = parrafos[0].split()
            parrafos = []
            current_parrafo = []
            current_length = 0

            # Procesar palabra por palabra para encontrar puntos seguidos de espacio o fin de oración
            for i, palabra in enumerate(palabras):
                current_parrafo.append(palabra)
                current_length += len(palabra) + 1  # +1 por el espacio

                # Si hemos alcanzado ~100 palabras y encontramos un punto (posible fin de oración)
                if len(current_parrafo) >= 100 and (
                    palabra.endswith(".")
                    or palabra.endswith("!")
                    or palabra.endswith("?")
                ):
                    ...
                    if not (
                        palabra.lower().endswith("dr.")
                        or palabra.lower().endswith("sr.")
                        or palabra.lower().endswith("sra.")
                        or palabra.lower().endswith("etc.")
                        or palabra.lower().endswith("ej.")
                        or palabra.lower().endswith("e.g.")
                        or palabra.lower().endswith("i.e.")
                        or palabra.lower().endswith("u.s.")
                        or palabra.lower().endswith("u.k.")
                        or palabra.lower().endswith("a.m.")
                        or palabra.lower().endswith("p.m.")
                    ):
                        # Completar el párrafo y empezar uno nuevo
                        parrafos.append(" ".join(current_parrafo))
                        current_parrafo = []
                        current_length = 0

            # Agregar el último párrafo si hay contenido restante
            if current_parrafo:
                parrafos.append(" ".join(current_parrafo))
    ...
    def _generar_paginas_articulos(
        self,
        site_dir: Path,
        noticias: List[Dict],
        metadata: Dict,
        ...
        for idx, noticia in enumerate(noticias, 1):
            ...
            article_url = (
                f"{metadata.get('domain', 'https://ejemplo.com')}/article_{idx}.html"
            )
            seo_meta_tags = self.seo_generator.generar_meta_tags_articulo(
                noticia, metadata, article_url, idx
            )
...
<body>
    ...
    <footer class="footer">
        <div class="container">
            ...
            if idx == 1:
                ...
                preloader_code = self.preloader_generator.generar_preloader_completo(
                    preloader_tipo, colores_preloader
                )
                article_html = self.preloader_generator.inyectar_en_html(
                    article_html, preloader_code
                )
    ...
    def _generar_paginas_legales(self, site_dir: Path, metadata: Dict):
        ...
        terms_html = self.legal_generator.generar_terminos_condiciones(
            site_name, domain
        )
        ...
        privacy_html = self.legal_generator.generar_politica_privacidad(
            site_name, domain
        )
    ...
    def ejecutar_flujo_completo(
        ...
        try:
            ...
            if site_dir.exists():
                self.log(
                    f"⚠️  El sitio site_{self.next_site_number} ya existe. Abortando para no sobrescribir.",
                    "WARNING",
                )
            ...
            noticias = self.paso_1_descargar_noticias(
                num_noticias=100, force_download=force_download
            )
            if not noticias:
            ...
            if offline_mode:
                ...
                self.log(
                    "⚠️  MODO OFFLINE ACTIVADO: Usando Spacy+NLTK en lugar de Blackbox/Gemini"
                )
                articulos_principales = self.paso_2_parafraseo_linguistico(
                    noticias[:100]
                )  # Procesar todas
            ...
            self.log(
                f"Total artículos: {len(todos_articulos)} ({len(articulos_principales)} destacados + {len(placeholders)} placeholders)"
            )
            ...
            noticias_categorizadas = self.paso_2_5_categorizar_noticias(
                todos_articulos, use_ai=not offline_mode
            )

            # Paso 2.6: Marcar y ordenar destacados
            noticias_categorizadas = self.featured_manager.marcar_destacados(
                noticias_categorizadas
            )
            noticias_categorizadas = self.featured_manager.ordenar_destacados_primero(
                noticias_categorizadas
            )

            separated = self.featured_manager.separar_destacados_y_placeholders(
                noticias_categorizadas
            )
            self.log(
                f"Destacados: {separated['stats']['total_featured']}, Placeholders: {separated['stats']['total_placeholders']}"
            )

            # Paso 3: Generar imágenes
            imagenes = self.paso_3_generar_imagenes(
                noticias_categorizadas, self.next_site_number
            )
            ...
            self.paso_8_generar_rss_feeds(
                noticias_categorizadas, sites_metadata[0], site_dir
            )

            # Paso 9: Generar páginas de categorías
            self.paso_9_generar_paginas_categorias(
                noticias_categorizadas, sites_metadata[0], site_dir
            )

            # Paso 10: Generar imágenes Open Graph
            self.paso_10_generar_og_images(
                noticias_categorizadas, sites_metadata[0], site_dir
            )
            ...
            self.log(
                f"Artículos principales (Blackbox Pro): {self.stats.get('articulos_principales', 20)}"
            )
            self.log(
                f"Placeholders (Gemini): {self.stats.get('placeholders_generados', 0)}"
            )
            ...
            self.log(f"")
            self.log(
                f"✨ MEJORA: Sistema paralelo {int((7 * 60) / tiempo_total if tiempo_total > 0 else 1)}x más rápido que el original"
            )
    ...
    def paso_8_generar_rss_feeds(
        self, noticias: List[Dict], site_metadata: Dict, site_dir: Path
    ):
        """
        ...
        try:
            feeds = self.rss_generator.generar_feeds_por_categoria(
                noticias, site_metadata, output_dir=str(site_dir)
            )
    ...
    def paso_9_generar_paginas_categorias(
        self, noticias: List[Dict], site_metadata: Dict, site_dir: Path
    ):
        """
        ...
        try:
            ...
            self.section_generator.generar_index_categorias(
                grouped, site_metadata, color_palette, str(index_path)
            )

            self.log(
                f"Generadas {len(grouped)} páginas de categorías + índice", "SUCCESS"
            )

        except Exception as e:
            self.log(f"Error generando páginas de categorías: {e}", "ERROR")

    def paso_10_generar_og_images(
        self, noticias: List[Dict], site_metadata: Dict, site_dir: Path
    ):
        """
        ...
        try:
            ...
            og_images = self.og_image_generator.generar_og_images_lote(
                noticias, site_metadata
            )

            self.log(
                f"Generadas {len(og_images)} imágenes Open Graph (1200x630)", "SUCCESS"
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Master Orchestrator - Generación Completa de Sitio"
    )
    parser.add_argument(
        "--verificar-dominios",
        action="store_true",
        help="Verificar disponibilidad de dominios",
    )
    parser.add_argument(
        "--api-whois",
        action="store_true",
        help="Usar APILayer WHOIS API (requiere APILAYER_API_KEY en .env)",
    )
    parser.add_argument(
        "--output-dir", type=str, default=None, help="Directorio de salida"
    )
    parser.add_argument(
        "--usar-cache",
        action="store_true",
        help="Usar noticias en cache en lugar de descargar nuevas",
    )
    parser.add_argument(
        "--offline",
        action="store_true",
        help="Usar modo offline (parafraseo lingüístico sin IA)",
    )
    ...
    orchestrator = MasterOrchestrator(
        output_base_dir=args.output_dir, usar_api_whois=args.api_whois
    )
