...
def main():
    ...
    for sitio in sitios_to_process:
        ...
        if args.dry_run:
            for i, post in enumerate(posts[:5], 1):
                ...
                print(
                    f"   📝 Post {i}: {post_id[:20] if len(post_id) > 20 else post_id} - {title}... ({fb_fecha})"
                )
            if len(posts) > 5:
        ...
        if success:
            ...
            if failed > 0:
                ...
                for err in errors[:3]:
                    print(
                        f"      - Post {err.get('post_id', 'N/A')}: {err.get('error', 'Unknown')}"
                    )
        else:
    ...
    if total_updated > 0:
        ...
        print()
        print(
            "Las miniaturas deberían verse correctamente en Facebook en unos minutos."
        )
