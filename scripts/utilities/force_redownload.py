#!/usr/bin/env python3
"""
Force re-download all article images
"""
...
for slug in os.listdir(SITES_DIR):
    ...
    print(f"   🗑️  Imágenes antiguas eliminadas")
    ...
    for i, article in enumerate(news_data[:21]):
        ...
        print(f"   📷 Descargando {i+1}/21: {article.get('title', '')[:40]}...")
        download_image(image_url, img_path)
    
    # Verify
    downloaded = len([f for f in os.listdir(assets_dir) if f.endswith('.jpg')])
    print(f"   ✅ {downloaded} imágenes descargadas\n")

print("✅ Todas las imágenes re-descargadas")
