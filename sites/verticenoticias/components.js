/**
 * Vértice Noticias - Componentes Compartidos
...
(function () {
    ...
    // ── PRELOADER: progress-circle SVG (Prussian Blue → Pacific Blue) ────────
    const PRELOADER_HTML = `
<div id="site-preloader" class="vn-preloader">
    <div class="vn-circle-wrap">
        <svg class="vn-svg" viewBox="0 0 100 100" xmlns="http://www.w3.org/2000/svg">
            <defs>
                <linearGradient id="vn-grad" x1="0%" y1="0%" x2="100%" y2="100%">
                    <stop offset="0%"   stop-color="#042548"/>
                    <stop offset="100%" stop-color="#4FB0C6"/>
                </linearGradient>
            </defs>
            <circle class="vn-track"    cx="50" cy="50" r="42" fill="none" stroke="rgba(79,176,198,0.12)" stroke-width="6"/>
            <circle class="vn-progress" cx="50" cy="50" r="42" fill="none" stroke="url(#vn-grad)"         stroke-width="6"
                    stroke-linecap="round" stroke-dasharray="264" stroke-dashoffset="264"
                    transform="rotate(-90 50 50)"/>
        </svg>
        <img src="${base}logo-header.png" alt="Vértice Noticias" class="vn-logo">
    </div>
    <p class="vn-preloader-text">Vértice Noticias</p>
</div>
<style>
.vn-preloader {
    position: fixed;
    ...
    background: linear-gradient(135deg, #021830 0%, #042548 60%, #063060 100%);
    display: flex;
...
.vn-preloader.loaded { opacity: 0; pointer-events: none; }
...
.vn-progress {
    animation: vn-fill 1.8s ease-in-out forwards;
}
...
#weather-temp{color:rgba(255,255,255,0.7);}
    ...
    function initFinanceTicker() {
        var el = document.getElementById('finance-ticker-value');
        if (!el) return;
        // Mostrar estáticos inmediatamente como fallback
            ...
            .then(function(data) {
                ...
                var liveItems = data.map(function(d) {
                    return {
                        label: d.NOMBRE,
                        value: d.VALOR,
                        change: d.CAMBIO || '0.00%',
                        up: d.TENDENCIA !== 'down'
                    };
                });
                // Reiniciar rotación con datos en vivo
                startFinanceRotation(liveItems);
            })
            .catch(function() { /* mantener estáticos */ });
    }
    ...
    <div class="container header-container">
        <div class="logo-section">
            <a href="${base}index.html" style="display:block;line-height:0;">
                <img src="${base}logo-header.png" alt="Vértice Noticias" class="site-logo" style="transform: scale(2); transform-origin: left center;">
            </a>
        </div>
        <nav class="main-nav">
...
<footer class="footer" style="padding:24px 0 12px;">
    <div class="container">
        <div class="footer-grid cols-3" style="gap:16px;">
            <div class="footer-column">
                <div class="footer-logo">
                    <img src="${base}logo-header.png" alt="Vértice Noticias">
                    <h3>Vértice Noticias</h3>
                </div>
                <p class="footer-about" style="font-size:0.82rem;line-height:1.5;margin:8px 0 10px;">Perspectivas únicas. Análisis profundo desde el vértice de la información.</p>
        ...
        <div class="footer-bottom" style="padding-top:10px;margin-top:0;">
            <p>&copy; 2026 Vértice Noticias. Todos los derechos reservados.</p>
    ...
    function initImageFallbacks() {
        ...
        function applyFallback(img) {
            ...
            img.addEventListener('error', function () {
                ...
                this.style.background = 'linear-gradient(160deg,#0A1628 0%,#1A2E4A 100%)';
