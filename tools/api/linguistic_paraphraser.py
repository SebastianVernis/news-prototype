...
# Intentar importar spacy y nltk (sin auto-instalar)
...
class LinguisticParaphraser:
    ...
    def __init__(self, model_size: str = "md"):
        ...
        self.available = _SPACY_AVAILABLE
        self.nlp = None
        if self.available:
            try:
        ...
        # Fallback básico si no hay modelos lingüísticos
        self.basic_synonyms = {
    ...
    def _load_spacy_model(self):
        ...
        try:
            ...
            raise RuntimeError(f"Modelo Spacy no disponible: {self.model_name}")
    ...
    def _ensure_nltk_data(self):
        ...
        try:
            ...
            raise RuntimeError("WordNet no disponible para NLTK")
    ...
    def paraphrase_text(self, text: str, change_threshold: float = 0.4) -> Dict:
        ...
        """
        if not text:
            return {"text": text, "changes_count": 0, "method": "empty"}

        if not self.available or self.nlp is None:
            words = text.split()
            changes = 0
            for i, word in enumerate(words):
                key = re.sub(r"[^\wáéíóúñÁÉÍÓÚÑ]", "", word.lower())
                if key in self.basic_synonyms:
                    replacement = self.basic_synonyms[key]
                    words[i] = replacement if word.islower() else replacement.capitalize()
                    changes += 1
            paraphrased = " ".join(words)
            return {
                "title": "Parafraseo Básico",
                "text": paraphrased,
                "original_text": text,
